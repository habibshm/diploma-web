...
<center>
<form name="fadminupdategame" method="post" action="adminUpdateGame.php" enctype="multipart/form-data">
<?php
// Include connection
...
$sqlgame = "SELECT * FROM games where GAME_ID='$gameid'";
$resultgame = mysqli_query($dbc, $sqlgame);
...
if (mysqli_num_rows($resultgame) > 0) {
    ...
    if($row = mysqli_fetch_assoc($resultgame))
	{
		print '<div style="margin-top: 100px;"><h1><font color="black">Details Of The Game.</font><h1></div>
				...
				<input type="text" id="GAME_ID" name="GAME_ID" value='.$row['GAME_ID'].'><br><br>
	
				<label for="GAME_NAME">Game Name</label>
				<input type="text" id="GAME_NAME" name="GAME_NAME" value='.$row['GAME_NAME'].'><br><br>
				
				<img width = 20% src="data:image/jpeg;base64,'.base64_encode( $row['GAME_PIC'] ).'"/><br>
				<label for="GAME_PIC">Select image to upload</label>
				<input style="background-color: white;" type="file" id="GAME_PIC" name="GAME_PIC">
	
				<label for="GAME_PRICE">Game Price</label>
				<input type="text" id="GAME_PRICE" name="GAME_PRICE" value='.$row['GAME_PRICE'].'>
		
				<label for="GAME_GENRE">Game Genre</label>
				<input type="text" id="GAME_GENRE" name="GAME_GENRE" value='.$row['GAME_GENRE'].'>
	
				<label for="GAME_DATE">Game Date</label>
				<input type="date" id="GAME_DATE" name="GAME_DATE" value='.$row['GAME_DATE'].'>
